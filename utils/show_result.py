...
def evaluate_results_binary(result, neg_id):
    total_p, total_g, right, total_right = 0, 0, 0, 0
    for _p, g in result:
        total_g += len(g)
        for p in _p:
            total_p += 1
    ...
    rec = 1.0 * right / total_g
    f1 = 2 * pre * rec / (pre + rec + 0.000001)
    out = 'Total Sample:%d, Total Pred: %d, Total Right_pred:%d,  Total Recall: %d\n' % (
    len(result), total_p, right,  total_g)
    out += ' Precision: %.3f, Recall: %.3f, F1: %.3f' % (pre, rec, f1)
    return out, f1
